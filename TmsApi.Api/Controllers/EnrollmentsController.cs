...
public class EnrollmentsController(
    ...
    IEnrollmentServices enrollmentService) : ControllerBase
{
        ...
        CancellationToken ct)
    {
        ...
        if (await enrollmentService.ExistsAsync(courseId, request.StudentId, ct))
        {
            return Conflict(new ProblemDetails
            {
                Title = "Student already enrolled",
                Detail = $"Student {request.StudentId} is already enrolled in '{course.Title}'.",
                Status = StatusCodes.Status409Conflict
            });
        }
